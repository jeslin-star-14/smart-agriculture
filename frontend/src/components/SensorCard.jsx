...
export default function SensorCard({ label, value, unit, icon, type }) {
  ...
  function getPercent() {
    ...
    const ranges = {
      ...
      soil: [0, 100], light: [0, 1000], ph: [0, 14], rain: [0, 1],
    };
  ...
  const hasValue = value !== null && value !== undefined && value !== "--" && value !== "--.-" && value !== "---" && value !== "-.-";

  const accentColor = { normal: "#00ff7f", warn: "#f59e0b", danger: "#ef4444" }[level];
  ...
  return (
    <div style={{ ...cardStyle, borderTopColor: hasValue ? accentColor : "#1e3326" }}>
      ...
      <div style={{ ...valueStyle, color: accentColor }}>{displayValue}</div>
      <div style={unitStyle}>{type === "rain" ? "status" : unit}</div>
      <div style={barTrack}>
        <div style={{ ...barFill, width: `${pct}%`, background: accentColor }} />
      </div>
    </div>
  );
}
...
const iconStyle = { fontSize: "1.9rem", marginBottom: "8px" };
const valueStyle = {
...
const barTrack = {
  height: "4px", background: "#1e3326", borderRadius: "2px",
