...
export default function LoginPage({ onLogin, onGoRegister }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError("Cannot reach server. Make sure backend is running (npm run dev in backend folder).");
    }
    setLoading(false);
  }
    ...
    <div style={S.page}>
      ...
      <div style={S.card}>
        ...
        <div style={{ textAlign:"center", fontSize:"0.83rem", color:"#64748b", marginTop:20 }}>
          ...
          <span style={{ color:"#3b82f6", cursor:"pointer", fontWeight:700 }} onClick={onGoRegister}>
            Create account
          </span>
        </div>

        <div style={S.demoBox}>
          🧪 Demo: <strong>demo@agri.com</strong> / <strong>demo1234</strong>
          <div style={{ fontSize:"0.68rem", color:"#334155", marginTop:3 }}>
            Works even without registering
          </div>
        </div>
      </div>
...
const S = {
  ...
  sub:{ fontSize:"0.83rem", color:"#64748b", marginBottom:8 },
  fieldWrap:{ display:"flex", flexDirection:"column", gap:6 },
  label:{ fontSize:"0.78rem", fontWeight:600, color:"#94a3b8", letterSpacing:"0.04em" },
  icon:{ position:"absolute", left:13, fontSize:"0.95rem", pointerEvents:"none", zIndex:1 },
  ...
  errorBox:{ background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.28)", borderRadius:10, padding:"11px 14px", fontSize:"0.83rem", color:"#fca5a5", lineHeight:1.6 },
  submitBtn:{ background:"linear-gradient(135deg,#2563eb,#0891b2)", color:"#fff", border:"none", borderRadius:12, padding:14, fontSize:"0.95rem", fontWeight:700, cursor:"pointer", fontFamily:"'Outfit',sans-serif", boxShadow:"0 6px 24px rgba(37,99,235,0.4)", transition:"all 0.2s" },
  demoBox:{ marginTop:16, background:"rgba(59,130,246,0.06)", border:"1px solid rgba(59,130,246,0.1)", borderRadius:10, padding:"10px 14px", fontSize:"0.76rem", color:"#475569", textAlign:"center" },
};