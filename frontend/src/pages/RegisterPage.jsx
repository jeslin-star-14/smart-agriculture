...
export default function RegisterPage({ onRegister, onGoLogin }) {
  ...
  function validate() {
    ...
    else if (strength < 4)     e.password = "Password too weak — check requirements below.";
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setErrors(p => ({ ...p, general: "Cannot reach server. Make sure backend is running." }));
    }
    setLoading(false);
  }
  ...
  if (success) return (
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"#060d1f", fontFamily:"'Outfit',sans-serif", gap:16 }}>
      ...
      <div style={{ color:"#94a3b8" }}>Welcome to AgriSense, {form.name.split(" ")[0]}!</div>
      <div style={{ color:"#475569", fontSize:"0.85rem" }}>Redirecting to dashboard…</div>
    </div>
  );
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.card}>
          ...
          <div>
            <Field label="Password" icon="🔒" error={errors.password}>
              <input name="password" type={showPass ? "text" : "password"} value={form.password}
                ...
                style={{ ...iStyle(!!errors.password), paddingRight:46 }} />
            ...
            {form.password && (
              <div style={{ marginTop:8 }}>
                ...
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
                  <span style={{ fontSize:"0.72rem", color:strengthColor, fontWeight:700 }}>{strengthLabel}</span>
                  <span style={{ fontSize:"0.68rem", color:"#475569" }}>{strength}/5 met</span>
                </div>
        ...
        <div style={{ textAlign:"center", fontSize:"0.83rem", color:"#64748b", marginTop:18 }}>
...
function Field({ label, icon, error, hint, checking, children }) {
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <label style={{ fontSize:"0.78rem", fontWeight:600, color:"#94a3b8", letterSpacing:"0.04em" }}>{label}</label>
      ...
      <div style={{ position:"relative", display:"flex", alignItems:"center" }}>
        <span style={{ position:"absolute", left:13, fontSize:"0.9rem", pointerEvents:"none", zIndex:1 }}>{icon}</span>
        {children}
      </div>
      {error && <span style={{ fontSize:"0.73rem", color:"#f87171", marginTop:1 }}>⚠ {error}</span>}
    </div>
  );
}
...
const S = {
  ...
  submitBtn:{ background:"linear-gradient(135deg,#2563eb,#0891b2)", color:"#fff", border:"none", borderRadius:12, padding:"13px 14px", fontSize:"0.95rem", fontWeight:700, cursor:"pointer", fontFamily:"'Outfit',sans-serif", boxShadow:"0 6px 24px rgba(37,99,235,0.4)", display:"flex", alignItems:"center", justifyContent:"center", gap:6 },
};